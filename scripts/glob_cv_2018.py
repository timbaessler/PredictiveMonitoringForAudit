...
if __name__ == "__main__":
    ...
    for classifier in list(["XGBoost"]):
        for pr in range(50):
            ...
            cv_respr = pd.read_csv(fname + 'cross_val_scores.csv', delimiter=";")
            cv_respr = cv_respr[cv_respr.rank_test_score==1]
        ...
        for pr in range(50):
            print(pr)
            fname = os.path.join(predict_path, classifier +"_"+str(pr).zfill(2) + "_")
            ...
            del log2
            del X
            clf = xgboost.XGBClassifier(**best_params, random_state=42)
