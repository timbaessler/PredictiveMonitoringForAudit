...
def load_train_test(log, filename, num_cols, static_cat_cols, dynamic_cat_cols, pr, train_idx, test_idx, read=False, tabnet_features=False):
    if os.path.exists(filename+"X_train.feather") and read:
        ...
        X, y = X.set_index("case:concept:name"), y.set_index("case:concept:name")
    else:
        ...
        del log2
    X_train = X_train.drop(columns="case:concept:name")
    X_test = X_test.drop(columns="case:concept:name")
    y_train = y_train.drop(columns="case:concept:name")
    y_test = y_test.drop(columns="case:concept:name")
    ...
    if tabnet_features:
        ...
        for i in range(len(Xcols)):
            if i < len(num_cols)*5:
                ...
                if Xcols[i].startswith("org:resource"):
                    org_resource = True
                    list_list.append(i)
                elif org_resource:
                    grouped_features.append(list_list)
                    list_list = list()
                    org_resource=False
                elif Xcols[i].startswith("concept:name"):
                    concept_name = True
                    list_list.append(i)
                elif concept_name:
                    grouped_features.append(list_list)
